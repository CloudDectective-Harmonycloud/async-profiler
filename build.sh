...
run_docker() {
  ...
  if [ -z "$(docker images | grep async-profiler-build | grep ${ARCH})" ];then
    docker build --build-arg IMAGE=${IMAGE} -t ${TAG} --platform ${PLATFORM} ${DIR}/docker
  fi

  # Start Docker Build
  docker run --rm -it --platform ${PLATFORM} -v ${DIR}:/data/src -v ${DIR}/docker/build.sh:/data/script/build.sh -v ${DIR}/docker/build_in_docker.sh:/data/script/build_in_docker.sh ${TAG} bash -eux /data/script/build_in_docker.sh
}
