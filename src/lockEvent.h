...
struct LockWaitEvent {
    ...
    void print() {
        printf("{\"threadId\":%d,\"threadName\":\"%s\", \"javaThreadId\":%d,\"waitTimestamp\":%ld,\"wakeTimestamp\":%ld,\"objectAddr\":\"%x\",\"lockType\":\"%s\",\"waitDuration\":%ld,\"waitThread\":%d,\"stack\":\"%s\"}",
        _native_thread_id, _thread_name.c_str(), _java_thread_id, _wait_timestamp, _wake_timestamp, _lock_object_address, _lock_type.c_str(), _wait_duration, _wait_thread_id, _stack_trace.c_str());
        printf("\n");
    }

    void log() {
        EventLogger::log("%ld!%ld!%d!%x!%s!%s!%ld!%d!%s", _wait_timestamp, _wake_timestamp, _native_thread_id, _lock_object_address, _lock_type.c_str(), _thread_name.c_str(), _wait_duration, _wait_thread_id, _stack_trace.c_str());
    }
};
#endif // _LOCKEVENT_H